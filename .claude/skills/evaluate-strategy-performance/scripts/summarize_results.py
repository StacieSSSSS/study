...
def main() -> int:
    ...
    summary_path = root / "walk_forward" / args.run_id / "summary.csv"
    windows_path = root / "walk_forward" / args.run_id / "windows.csv"
    missing = [str(path) for path in [metrics_path, summary_path, windows_path] if not path.exists()]
    if missing:
